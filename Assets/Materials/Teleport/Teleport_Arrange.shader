...
Shader "HOTATE/Teleport/Teleport_Arrange"
{
	Properties
	{
		...
		Pass
		{
		   ...
		   // ※ 接空間からローカル空間への変換の逆行列で、ローカルのライトを接空間に変換する
			...
			h2d_const HSConst(InputPatch<v2h,INPUT_PATCH_SIZE> i)
			{
				...
				o.normal[0] = i[0].normal;
				o.normal[1] = i[1].normal;
				o.normal[2] = i[2].normal;

				o.lightDir[0] = i[0].lightDir;
				o.lightDir[1] = i[1].lightDir;
				o.lightDir[2] = i[2].lightDir;
				return o;
			}
			...
			d2g DS(h2d_const hs_const_data, const OutputPatch<h2d_main, OUTPUT_PATCH_SIZE> i, float3 bary:SV_DomainLocation)
			{
				...
				o.lightDir = i[0].lightDir * bary.x + i[1].lightDir * bary.y + i[2].lightDir * bary.z;;
				return o;
			}
			...
			void geom(triangle d2g input[3],inout TriangleStream<g2f> outStream)
            {
				...
				for(int i; i < 3; i++){
					...
					output[i].lightDir = input[i].lightDir;
			...
			fixed4 frag (g2f i, v2h j) : SV_Target
			{
				...
				// NormalTest 
				//return float4(UnpackNormal(tex2D(_BumpMap, i.uv)), 1);

				//return float4(i.uv,1,1);
				//return float4(light, 1); 
				//return float4(j.lightDir, 1);

				float3 light = normalize(j.lightDir);

				//return float4(light, 1);
				float diff = max(0, dot(normal, light));
				return diff * _Color * col;
